                                        ...
                                        <select name="items[0][batch_id]" class="form-select batch-select" required onchange="updatePrice(this, 0)">
                                            ...
                                            @foreach ($batches as $batch)
                                                <option value="{{ $batch->id }}" 
                                                        ...
                                                        data-price="{{ $batch->price_per_unit ?? 0 }}">
                                                    {{ $batch->medicine->name }} - Batch: {{ $batch->batch_number }} 
                                                    (Stock: {{ $batch->quantity }}, Exp: {{ date('d/m/Y', strtotime($batch->expiry_date)) }})
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.calculateItemTotal = function(index) {
        ...
        const priceInput = document.querySelector(`input[name="items[${index}][price_per_unit]"]`);
        const totalInput = quantityInput.closest('.item-row').querySelector('.item-total');
